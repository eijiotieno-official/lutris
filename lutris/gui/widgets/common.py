...
from lutris.gui.widgets.utils import clear_box_children, get_widget_window, open_uri
...
class FileChooserEntry(Gtk.Box):  # type:ignore[misc]
    ...
    def __init__(
        ...
        browse_button = Gtk.Button(icon_name="view-more-horizontal-symbolic")
        ...
        self.open_button = Gtk.Button(icon_name="folder-symbolic")
    ...
    def get_filechooser_dialog(self):
        """Return an instance of a Gtk.FileDialog configured for this widget"""
        dialog = Gtk.FileDialog(title=self.title)
        dialog.set_initial_folder(Gio.File.new_for_path(self.get_default_folder()))
        return dialog
    ...
    def on_browse_clicked(self, _widget):
        ...
        parent = get_widget_window(self)

        def on_finished(_dialog, result):
            try:
                if self.action == Gtk.FileChooserAction.SAVE:
                    file = _dialog.save_finish(result)
                elif self.action == Gtk.FileChooserAction.SELECT_FOLDER:
        ...
        if self.action == Gtk.FileChooserAction.SAVE:
            file_chooser_dialog.save(parent, None, on_finished)
        elif self.action == Gtk.FileChooserAction.SELECT_FOLDER:
            file_chooser_dialog.select_folder(parent, None, on_finished)
        else:
            file_chooser_dialog.open(parent, None, on_finished)
    ...
    def on_entry_changed(self, widget):
        ...
        if self.warn_if_ntfs and LINUX_SYSTEM.get_fs_type_for_path(path) == "ntfs":
            ...
            warning_image = Gtk.Image.new_from_icon_name("dialog-warning")
    ...
    def on_focus_out(self, _controller, *_args):
        self.normalize_path()
        self.detect_changes()
    ...
    def clear_warnings(self):
        ...
        clear_box_children(self, keep_first=1)
